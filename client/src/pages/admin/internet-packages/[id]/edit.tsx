...
export default function AdminInternetPackageEdit() {
  const [, params] = useRoute("/admin/internet-packages/:id/edit");
  ...
  }, [token, packageId, setLocation]);

  const fetchPackage = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/admin/internet-packages/${packageId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        throw new Error("Paket bulunamadı");
      }
      const pkg: InternetPackage = await res.json();
      const features = pkg.features ? JSON.parse(pkg.features).join("\n") : "";
      setFormData({
        name: pkg.name,
        speed: pkg.speed.toString(),
      ...
      toast({
        title: "Hata",
        description: error.message || "Paket yüklenemedi",
        variant: "destructive",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        title: "Hata",
        description: error.message || "Paket güncellenemedi",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    return (
      <AdminLayout>
        <div className="space-y-6">
          <div>
            <h1 className="text-3xl font-bold">Paket Düzenle</h1>
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">Paket Düzenle</h1>
            <p className="text-muted-foreground">Paket bilgilerini düzenleyin</p>
          </div>
        </div>
